// $Id: lyutils.cpp,v 1.1 2017-10-30 01:03:20-07 - - $
...
astree* parser::fun(astree* one, astree* two, astree* three) {
   astree* front;
   if(strcmp(three->lexinfo->c_str(), ";") == 0) {
      front = new astree(TOK_PROTOTYPE, one->lloc, "");
      front->adopt(one, two);
   }else{
      front = new astree(TOK_FUNCTION, one->lloc, "");
      front->adopt(one, two);
      front->adopt(three);
   }
   return front;
